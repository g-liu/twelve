...
/**
 * A NamedNote represents a defined pitch with a name. This class is immutable.
...
public class NamedNote extends AbstractNote {
	...
	/**
	 * Constructs a new Node belonging to the specified PitchClass and having
	 * the specified register.
	 * 
	 * @param pc
	 *            the PitchClass that this NamedNote belongs to. Must not be
	 *            null.
	 */
	public NamedNote(PitchClass pc) {
		...
		otherStringRep = otherBuilder.length() > 0 ? otherBuilder.toString()
				: stringRep;
	}
	 ...
	 * 
	 * @param note
	 *            the String representing the NamedNote to be created. The
	 *            format of the Note should be as follows:
	 *            <ul>
	 *            <li>First character: [A-G]</li>
	 *            <li>Second character (optional): A note modifier, one of the
	 *            following: b or ♭ (flat) # or ♯ (sharp) ♮ (natural) x, ##, or
	 *            ♯♯ (double sharp) bb or ♭♭ (double flat)</li>
	 *            </ul>
	 * 
	 *            If the Note contains a double-sharp or a double-flat, the
	 *            representation will be normalized such that the note will not
	 *            have said modifier. For example, A## will be normalized to B
	 *            (natural).
	 * @throws IllegalArgumentException
	 *             if the parameter is not of a valid format.
	 */
	public NamedNote(String note) throws IllegalArgumentException {
		...
		if (noteParse.length > 1 && noteParse[1].length() > 0) {
			...
			case "bb":
				...
				break; // double flat
			default:
				throw new IllegalArgumentException("Invalid note modifier "
						+ noteParse[1]);
			}
		}
	 ...
	 * 
	 * @param note
	 *            String representation of the NamedNote, e.g "G#"
	 * @return
	 */
	private String getEnharmonic(String note) {
		...
		if (noteTokens.length > 1) {
			...
			case "#":
			case "♯":
				enh.append(NoteLetter.nextNote(noteTokens[0]));
			...
			case "b":
			case "♭":
				enh.append(NoteLetter.previousNote(noteTokens[0]));
			...
			case "♯♯":
			case "##":
				enh.append(NoteLetter.nextNote(noteTokens[0])); // double sharp
			...
			case "bb":
			case "♭♭":
				enh.append(NoteLetter.previousNote(noteTokens[0])); // double
																	// flat
				switch (noteTokens[0].toUpperCase()) { // may have to add flats
				case "C":
				case "F":
	...
	/**
	 * Returns the NamedNote resulting from transposing this note by a certain
	 * interval.
	 * 
	 * @param interval
	 *            the interval to which to transpose the NamedNote
