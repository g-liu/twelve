...
/**
 * Represents the seven note letters in Conventional Western Music Notation.
 * 
 * @author Geoffrey Liu
 */
public enum NoteLetter {
	 ...
	 * 
	 * @param letter
	 *            the character representing the note letter
	 */
	 ...
	 * @param current
	 *            the current note
	 * @return the NoteLetter enum representing the note letter that succeeds
	 *         the current note letter
	 */
	public static NoteLetter nextNote(NoteLetter current) {
		return NoteLetter.values()[(current.ordinal() + 1)
				% NoteLetter.values().length];
	}
	 ...
	 * @param current
	 *            the current note
	 * @return the NoteLetter enum representing the note letter that precedes
	 *         the current note letter
	 */
	public static NoteLetter previousNote(NoteLetter current) {
		return NoteLetter.values()[(current.ordinal()
				+ NoteLetter.values().length - 1)
				% NoteLetter.values().length];
	}
}
